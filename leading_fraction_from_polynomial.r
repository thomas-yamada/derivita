...
#split apart the polynomial into coefficient/constant and variable/term
...
#take each coefficient/constant and put them into a list
const_list := [c1,c2,c3,c4,c5,c6,c7,c8,c9,c10]
...
#multiplies the coefficients/constants with the left behind numberator, pretty much only changing the constant, because thats the only one with a left behind numberator
altered1_5 := list_transform(altered, simplify(nth_item(index, altered)*nth_item(index, const_list)))
#remove "undefined"
altered2 := list_filter(altered1_5, not(is_match(:x, item)))
#isolate variables by multiplying original terms by the origianl coefficient/constant, then multiply that by the reciprocal of the coefficient
isolate_var := [compute((t1*c1)/(nth_item(1, altered2))), compute((t2*c2)/(nth_item(2, altered2))), compute((t3*c3)/(nth_item(3, altered2))), compute((t4*c4)/(nth_item(4, altered2))), compute((t5*c5)/(nth_item(5, altered2))), compute((t6*c6)/(nth_item(6, altered2))), compute((t7*c7)/(nth_item(7, altered2))), compute((t8*c8)/(nth_item(8, altered2))), compute((t9*c9)/(nth_item(9, altered2))), compute((t10*c10)/(nth_item(10, altered2)))]
#remove 1
